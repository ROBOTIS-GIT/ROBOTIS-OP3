...
#include <QMessageBox>
#include <iostream>
#include "../include/op2_demo/main_window.hpp"
...
//    qnode_thor3.log(QNodeOP2::Info, "Apply new FT config");
//}
//void MainWindow::on_button_ft_save_clicked(bool check)
//{
//    qnode_thor3.initFTCommand("ft_save");
//    qnode_thor3.log(QNodeOP2::Info, "Save FT config data.");
//}
...
void MainWindow::on_head_center_button_clicked(bool check)
{
    ...
    qnode_thor3.log(QNodeOP2::Info, "Go Head init position");
    setHeadAngle(0, 0);
}
...
void MainWindow::setMode(bool check)
{
    ...
    for(int ix = 0; ix < _combo_children.length(); ix++)
    {
        ...
        qnode_thor3.log(QNodeOP2::Info, _stream.str());
    }

    // no control
    if(_control_msg.joint_name.size() == 0) return;

    qnode_thor3.log(QNodeOP2::Info, "set mode");
...
void MainWindow::initMotionUnit()
{
    ...
    for(std::map< int, std::string >::iterator _iter = qnode_thor3.motion_table.begin(); _iter != qnode_thor3.motion_table.end(); ++_iter)
    {
        ...
        //hotkey
        std::map<int, int>::iterator _short_iter = qnode_thor3.shortcut_table.find(_motion_index);
        if(_short_iter != qnode_thor3.shortcut_table.end())
...
{
    qnode_thor3.setControlMode(mode_name.toStdString());
}
