...
JointTrajectoryControlModule::JointTrajectoryControlModule()
  : control_cycle_msec_(0),
    ...
    LEFT_ELBOW_INDEX(6),
    DEBUG(false),
    follow_joint_trajectory_server_(ros_node_, "fullbody_controller/follow_joint_trajectory_action", false),
    follow_action_initialized_(false)
{
  enable_ = false;
  module_name_ = "joint_trajectory_control_module";
...
JointTrajectoryControlModule::~JointTrajectoryControlModule()
{
  follow_joint_trajectory_server_.shutdown();
}

void JointTrajectoryControlModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  /* get Param */
  ros_node_.param<double>("/robotis/joint_trajectory_control/default_moving_angle", default_moving_angle_, default_moving_angle_);
  ros_node_.param<bool>("/robotis/joint_trajectory_control/check_collision", check_collision_, check_collision_);

  /* publish topics */
  status_msg_pub_ = ros_node_.advertise<robotis_controller_msgs::StatusMsg>("/robotis/status", 0);

  /* action server */
  follow_joint_trajectory_server_.registerGoalCallback(boost::bind(&JointTrajectoryControlModule::onFollowJointTrajectoryActionGoal, this));
  follow_joint_trajectory_server_.registerPreemptCallback(boost::bind(&JointTrajectoryControlModule::onFollowJointTrajectoryActionPreempt, this));
  follow_joint_trajectory_server_.start();
}

void JointTrajectoryControlModule::onFollowJointTrajectoryActionGoal()
{
  if (enable_ == false)
  {
    ROS_INFO_THROTTLE(1, "JointTrajectory control module is not enable.");
  ...
  trajectory_msgs::JointTrajectoryPoint traj_point = goal->trajectory.points[0];
  ...
  // moving time
  moving_time_ = traj_point.time_from_start.toSec();

  // set target joint angle
  target_position_ = goal_position_;        // default is goal position

  for (int ix = 0; ix < goal->trajectory.joint_names.size(); ix++)
  {
    std::string joint_name = goal->trajectory.joint_names[ix];
    ...
    if (joint_it != using_joint_name_.end())
    {
      ...
      // set target position
      target_position = traj_point.positions[ix];
      ...
      if (DEBUG)
        std::cout << "joint : " << joint_name << ", Index : " << joint_index << ", Angle : " << traj_point.positions[ix]
  ...
  // generate trajectory
  tra_gene_thread_ = new boost::thread(boost::bind(&JointTrajectoryControlModule::jointTraGeneThread, this));
  delete tra_gene_thread_;
}

void JointTrajectoryControlModule::onFollowJointTrajectoryActionPreempt()
{
  follow_joint_trajectory_server_.setPreempted();
}

void JointTrajectoryControlModule::process(std::map<std::string, robotis_framework::Dynamixel *> dxls,
                                  std::map<std::string, double> sensors)
{
       ...
       state_it != result_.end(); state_it++)
  {
    ...
    follow_joint_trajectory_feedback.joint_names.push_back(joint_name);
    follow_joint_trajectory_feedback.desired.positions.push_back(goal_position);
    follow_joint_trajectory_feedback.actual.positions.push_back(goal_position);
...
void JointTrajectoryControlModule::stop()
{
  tra_lock_.lock();
...
bool JointTrajectoryControlModule::isRunning()
{
  return is_moving_;
}

void JointTrajectoryControlModule::onModuleEnable()
{
  is_updated_ = false;
  is_blocked_ = false;
  r_min_diff_ = 0.07;
...
void JointTrajectoryControlModule::onModuleDisable()
{

}

void JointTrajectoryControlModule::startMoving()
{
  is_moving_ = true;
...
void JointTrajectoryControlModule::finishMoving()
{
  // send action result
  if (is_moving_)
  {
...
void JointTrajectoryControlModule::stopMoving()
{
  // init value
  calc_joint_tra_ = goal_position_;
  tra_size_ = 0;
 ...
 */
Eigen::MatrixXd JointTrajectoryControlModule::calcMinimumJerkTraPVA(double pos_start, double vel_start, double accel_start,
...
bool JointTrajectoryControlModule::checkSelfCollision()
{
...
bool JointTrajectoryControlModule::getDiff(OP3KinematicsDynamics *kinematics, int end_index, int base_index, double &diff)
{
...
void JointTrajectoryControlModule::jointTraGeneThread()
{
  tra_lock_.lock();
...
void JointTrajectoryControlModule::publishStatusMsg(unsigned int type, std::string msg)
{
  ...
  status_msg.module_name = "Joint Trajectory Control";
